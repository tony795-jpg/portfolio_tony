...
(Note: Before we begin, I have to state that I somehow sourced an old PC that someone has actually given away for free and repurpose it as a home server)
### Architecture overview
The architecture looks something like this: 
#### Frontend (cloud side)
- GitHub repository (this exact one you are looking at): The central vault for the code. Holds no files, keeping it lightweight and functional.
- Netlify: I use this as the production deployment layer. It listens for GitHub updates, searches for index.html, and updates the live website globally in around a few seconds. 
#### Backend (home side)
...
#### Network side: 
- Tailscale WireGuard mesh: This creates a private virtual network overlay. Set up Tailscale funnel so that when the frontend requests an image, Tailscale tunnels into the server and beams the images to the user's screen. (tl/dr: I use this to make the server communicate with the website whenever needed)
...
Night 2:
* Why my HTML is broken because a copy-paste error turned `class=""` into `ass=""`. (quite funny I think)
